package spider_fla
{
   ...
   [Embed(source="/_assets/assets.swf", symbol="symbol3812")]
   public dynamic class cnt_189 extends MovieClip
   {
      ...
      {
         var _loc2_:String = "http://tracking.battleon.com/click.asp?game=AW&campaign=DEATHAQW_" + arrAds[iAdIndex].sType.toUpperCase();
      ...
      public function loadAd() : void
      {
         ...
         {
            _loc1_ = (rootClass.date_server.getTime() - rootClass.world.myAvatar.objData.dCreated.getTime()) / 1000;
            _loc2_ = _loc1_ / (60 * 60);
            _loc3_ = 0;
            while(_loc3_ < arrAds.length)
            {
               if(arrAds[_loc3_].sType == "member" && rootClass.world.myAvatar.isUpgraded() || arrAds[_loc3_].sType == "aeextra" && int(rootClass.world.myAvatar.objData.iAge) < 13 || arrAds[_loc3_].sType == "dragonhero" && (_loc2_ >= 72 || rootClass.world.myAvatar.isUpgraded()))
               {
                  arrAds.splice(_loc3_,1);
                  _loc3_--;
               }
               _loc3_++;
            }
            ...
            _loc4_ = new Loader();
            _loc4_.load(new URLRequest(rootClass.sFilePath + arrAds[iAdIndex].sURL));
            mcContent.mcAd.addChild(_loc4_);
      ...
      public function onCheckBoxClick(param1:Event) : void
      {
